...
# Copied from https://github.com/fla-org/flame
...
def main(job_config: JobConfig):
    ...
    if os.path.exists(os.path.join(job_config.job.dump_folder, 'config.json')):
        logger.info(f"Use saved config from {job_config.job.dump_folder}")
        model_config = AutoConfig.from_pretrained(job_config.job.dump_folder)
    else:
        logger.info(f"Loading model config from {job_config.model.config}")
        model_config = AutoConfig.from_pretrained(job_config.model.config)
        model_config.save_pretrained(job_config.job.dump_folder)
    job_config.model.model_config = model_config.to_dict()
    ...
    # Word-around when init_checkpoint_path not working; may not work with FSDP
    if os.environ.get("INIT_MODEL_PATH", "") != "":
        m_path = os.environ["INIT_MODEL_PATH"]
        m1 = AutoModelForCausalLM.from_pretrained(m_path, trust_remote_code=True)
        model.load_state_dict(m1.state_dict(), strict=False)
        del m1

        # s1 = list(model.state_dict().keys())
        # s2 = list(m1.state_dict().keys())
        # print("Unfilled keys:", set(s1) - set(s2))
        # print("Unused keys:", set(s2) - set(s1))
        #
        # print(model.model.layers[-1].attn.router.conv_stack.proj.bias)
    ...
    logger.info(
        f"{color.green}  Number of tokens per sequence = {job_config.training.seq_len:,} (context length {job_config.training.context_len}), varlen: {job_config.training.varlen}"
    )
    logger.info(
    ...
    use_lte = initialize_lte(model, train_state, job_config, logger)
    ...
    with (
        ...
        while train_state.step < job_config.training.steps:
            ...
            lte_state = prepare_lte_step(model, train_state, logger) if use_lte else None
            ...
            for _ in range(job_config.training.gradient_accumulation_steps):
                ...
                if parallel_dims.pp_enabled:
                    ...
                    with train_context(optional_context_parallel_ctx):
                        ...
                        if use_lte:
                            length = apply_lte_loss(output, lte_state)
            ...
            # Update L1 lambda
            if use_lte:
                lte_log = update_lte_lambda(
                    model, train_state, lte_state, length, parallel_dims, world_mesh, logger
                )
            ...
            if metric_logger.should_log(train_state.step):
                ...
                if use_lte:
                    metrics.update(make_lte_metrics(train_state.step, lte_state, lte_log, model))
